...
d3.queue()
  ...
  .defer(d3.csv,"CGT470Dataset.csv",function(d) { data.set(d.County, +d.PercentHSDiplomasEarned); }) //
...
d3.queue()
  .defer(d3.json,"indycounty.json")
  .defer(d3.csv,"in_out_2016.csv",function(d) { data2.set(d.description, +d.LabWorkRatio); }) //row.description, d means role in this part cam - uses two data maps
  // data2.set(d.description, +d.Live_in)
  .await(ready); //CHANGE DATA
  function ready(error,countyMap){
    if (error) throw error;
...
///////////////////// 
//How to make circles on the data map

/*var new_data = [{Name: "DeKalb", value: 300}, {Name: "Marion", Value: 500}]

var circle  = svg.selectAll("circle");

circle.data(new_data_.enter().append("circle").attr("cx",function(d) {return d.Value}).attr("cy",600).attr("r",10).style("fill",purple).on("click", function(d){console.log(d.NAME)}))
*/
